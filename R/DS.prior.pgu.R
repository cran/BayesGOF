DS.prior.pgu <- function(vec.counts, max.m = 8, start.par, iter.c = 200,
			...
			max.cnt <- max(cnt.vec)
			#out$max.lambda <- 100
			theta.vals <- seq(0.001, max.cnt, length.out = B) 
			d.x <- dgamma(theta.vals, start.par[1], scale = start.par[2]) #g instead of d
			...
			if(smooth.crit == "BIC"){
				...
				for(j in 1:max.m){
					...
					for(i in 1:iter.c){ ##uses just iter in base file
						c.L.new <- Reduce.LP.coef.univ(wght.mat=wght.loop, c.vec, u.loop, leg.mat.j, freq.vec = freq.vec, k.tot = k)
			...
			if(sum(dev.m)==0){
				...
				Leg.mat <- LP.basis.gamma(theta.vals, start.par, out$m.val)
