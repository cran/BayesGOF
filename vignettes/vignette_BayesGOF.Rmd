...
**Step 4.** Extract the parameters for the *corrected* priors: $\widehat{\pi}$ and $\widehat{\breve{\pi}}$:
...
The DS prior in its maximum entropy representation is: $$\widehat{\breve{\pi}}(\theta) = g(\theta; \mu, \tau^2)\exp\Big[-0.15 + 0.26T_3(\theta;G)-0.28T_4(\theta;G)+0.46T_5(\theta;G)\Big] $$
...
**Step 3.** We now extract the parameters for the *corrected* prior $\widehat{\breve{\pi}}$ and plot it, along with the original $g$:

```{r}
arsn.ds
```

The resulting equation for $\widehat{\breve{\pi}}(\theta)$ is: $$\widehat{\breve{\pi}}(\theta) = g(\theta; \mu, \tau^2)\exp\Big[-0.48 - 0.65T_2(\theta;G)-0.71T_3(\theta;G)+0.42T_4(\theta;G)\Big] $$
...
Based on our results, we find two significant modes.  Therefore, the prior shows structured heterogeneity and requires both modes to describe the distribution and its two groups.  In this cases, though, we are looking to estimate the consensus value of the measurand and its uncertainty.  Therefore, we would select the dominant mode.  To find the dominant mode, we plot the macroinference results:
...
Based on the plot, the dominant mode and our consensus value for the measurand is 13.6 with a standard error (using smooth bootstrap) of 0.25. 
